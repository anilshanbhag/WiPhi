...
for file_idx=1:length(files_all)
    filename{1}=sprintf('Data\\5Dec_m1_%d.dat',files_all(file_idx));
    filename{2}=sprintf('Data\\5Dec_m2_%d.dat',files_all(file_idx));
    ant_num=1;
    ...
    for ch=1:1:length(chan)
        ...
        for i=1:1:length(idx1)
            ...
            intercept2{ch}(i)=mean(h1./h2);
            h_real{ch}(i)=mean(h1);            
            %intercept2{ch}(i)=mean(h2);
...
%     end
    ...
    npaths=2;
    h1= @(x) optim_fun_lin_distance(x,h.',lambda,options);
